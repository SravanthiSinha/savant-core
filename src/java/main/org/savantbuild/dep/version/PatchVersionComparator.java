...
/**
 * <p>
 * This class is an artifact compatibility checker for MAJOR.MINOR.PATCH versioning
 * schemes. MAJOR and MINOR versions are by definition incompatible upgrades of the API;
 * PATCH versions retain source and binary compatibility with older minor versions;
 * </p>
 *
...
public class PatchVersionComparator implements VersionComparator {
   ...
   * @return Null is returned if either of the versions is null or if they are not on the same
   *         major and minor line. Otherwise, this returns the version that has the highest value.
   */
  public String determineBestVersion(String previousVersion, String currentVersion) {
    if (previousVersion.equals(currentVersion)) {
      ...
      if (currentVer.getMajor() != previousVer.getMajor()) {
        return null;
      }

      if (currentVer.getMinor() != previousVer.getMinor()) {
        return null;
      }
