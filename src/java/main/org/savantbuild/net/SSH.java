...
/**
 * <p>
 * This class uses teh JSCH library to connect into the SSH protocol
...
public class SSH {
   ...
   * @throws BuildException If the SSH command failed or any errors were found.
   */
  public String execute(String command) throws BuildException {
    try {
      ...
      if (thread.isAlive()) {
        // ran out of time
        throw new BuildException("SSH command [" + command + "] timed out.");
      }

      int code = exec.getExitStatus();
      if (code != 0) {
        throw new BuildException("Unable to execute SSH command [" + command + "] due to [ " + error.toString() + "]");
      }
