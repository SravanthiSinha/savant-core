...
import org.savantbuild.dep.workflow.process.URLProcessHandler;
...
public class GraphBuilderTest {
  ...
  public void transitiveDependencies() throws Exception {
    ...
    assertEquals(graph.getAllGraphNodesValues().size(), 7);
    ...
    GraphNode node = graph.getGraphNode(transArtifact.getId());
    ...
    List<GraphPath<ArtifactID>> paths = graph.getPaths(transArtifact.getId(), id);
    ...
    paths = graph.getPaths(transArtifact.getId(), id);
    assertEquals(paths.size(), 1);
    assertEquals(paths.get(0).getPath().size(), 2);
    assertEquals(paths.get(0).getPath().get(0), transArtifact.getId());
    assertEquals(paths.get(0).getPath().get(1), id);
  ...
  public void multipleVersion() throws Exception {
    ...
    GraphNode node = graph.getGraphNode(upgrade.getId());
    assertEquals(upgrade.getId(), node.getValue());

    ArtifactID id = new ArtifactID("org.savantbuild.test", "dependencies", "dependencies", "jar");
    List<GraphPath<ArtifactID>> paths = graph.getPaths(upgrade.getId(), id);
    assertEquals(paths.size(), 1);
    assertEquals(paths.get(0).getPath().size(), 2);
    ...
    id = new ArtifactID("org.savantbuild.test", "major-compat", "major-compat", "jar");
    paths = graph.getPaths(upgrade.getId(), id);
  ...
  public void integration() throws Exception {
    ...
    // Including the project node.
    assertEquals(graph.getAllGraphNodesValues().size(), 2);
    ...
    GraphNode<ArtifactID, ArtifactLink> node = graph.getGraphNode(integration.getId());
    assertEquals(integration.getId(), node.getValue());

    GraphLink<ArtifactID, ArtifactLink> link = node.getInboundLink(graph.getGraphNode(graph.getRoot().getId()));
