...
public class LocalSubVersion {
  ...
  public static String getProjectURL(File dir) {
    try {
      ...
      if (!dir.isDirectory()) {
        throw new BuildException("Not a directory [" + dir + "]");
      }

      if (!new File(dir, ".svn").isDirectory()) {
        throw new BuildException("Not a valid SubVersion working directory [" + dir + "]");
      }
      ...
      throw new BuildException("Unable to get the SVN info for the directory [" + dir.getAbsolutePath() + "]", e);
    }
  }

  /**
   * @param dir The working directory to get the SVN info for.
   * @return The URL of the working directory's SVN repository minus the trunk, tags or branches
   *         part of the URL.
   */
  ...
  public static String determineLocation(String url) {
    ...
    if (url.contains("branches")) {
      ...
      throw new BuildException("Invalid SubVersion URL [" + url + "]. It must use the standard SubVersion trunk, tags, and branches scheme");
    }
  ...
  public static void doStatus(File path, StatusHandler handler) {
    ...
    try {
      ...
      throw new BuildException("Unable to get status information for the path [" + path.getAbsolutePath() + "]", e);
    } finally {
